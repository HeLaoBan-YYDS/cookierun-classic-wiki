...
export async function generateSlugMetadata({
  ...
  if (slug.length === 1 && CONTENT_TYPES.includes(slug[0])) {
    ...
    return {
      ...
      openGraph: { title, description, url: `${siteUrl}${canonical}`, images: [`${siteUrl}/images/hero.webp`] },
    };
  }
  ...
  return {
    title: `${item.metadata.title} — CookieRun Classic Wiki`,
 ...
 *  - 2+ 段 → 文章详情
 */
export async function SlugView({
  locale,
  slug,
  navGroups,
}: {
  ...
  if (slug.length === 1) return <NavigationView locale={locale} contentType={slug[0]} navGroups={navGroups} />;
  return <DetailView locale={locale} contentType={slug[0]} slug={slug.slice(1)} navGroups={navGroups} />;
}

async function NavigationView({ locale, contentType, navGroups }: { locale: Locale; contentType: string; navGroups: NavGroup[] }) {
  ...
  const listData = {
    ...
    "@type": "ItemList",
    name: `${contentType} — CookieRun Classic Wiki`,
    itemListElement: items.map((item, index) => ({
      "@type": "ListItem",
      position: index + 1,
      url: `${siteUrl}${localizedPathname(`/${contentType}/${item.slug}`, locale)}`,
      name: item.metadata.title,
    })),
  };

  const sectionTitle =
    (messages as unknown as Record<string, Record<string, string>>)[contentType]?.overviewTitle ||
    contentType.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
  const sectionDesc =
    (messages as unknown as Record<string, Record<string, string>>)[contentType]?.overviewDescription || "";
...
async function DetailView({ locale, contentType, slug, navGroups }: { locale: Locale; contentType: string; slug: string[]; navGroups: NavGroup[] }) {
  if (!CONTENT_TYPES.includes(contentType)) notFound();
  const messages = (await getMessages({ locale })) as Messages;
  ...
  const articleData = {
    ...
    author: { "@type": "Organization", name: "CookieRun Classic Wiki" },
    publisher: { "@type": "Organization", name: "CookieRun Classic Wiki", logo: { "@type": "ImageObject", url: `${siteUrl}/android-chrome-512x512.png` } },
  };
    ...
    itemListElement: [
      {
        ...
        position: 1,
        name: "Home",
        item: `${siteUrl}${localizedPathname("/", locale) === "/" ? "" : localizedPathname("/", locale)}`,
      },
      { "@type": "ListItem", position: 2, name: sectionLabel, item: `${siteUrl}${localizedPathname(`/${contentType}`, locale)}` },
      { "@type": "ListItem", position: 3, name: item.metadata.title, item: `${siteUrl}${canonical}` },
    ],
  };
...
function SmallCard({ title, description, href, icon }: { title: string; description: string; href: string; icon?: React.ReactNode }) {
  return (
    <Link href={href} className="block rounded-2xl border border-border bg-card/70 p-5 transition hover:border-[hsl(var(--nav-theme-light))]">
